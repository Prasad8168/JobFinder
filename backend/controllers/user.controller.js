    ...
    try {
        ...
        if (file) {
            ...
            const cloudResponse = await cloudinary.uploader.upload(fileUri.content, {
                ...
                unique_filename: false, // No random hash in URL
                overwrite: true, // Replace file if already exists
                type: "upload", // Make file public
                flags: "attachment:off",
...
export const getBookmark = async (req, res) => {
    try {
        const user = await User.findById(req.id)
            .populate({
                path: "bookmarks",  // Populate jobs
                populate: {
                    path: "company",  // Populate company inside jobs
                    model: "Company",
                    select: "name logo location"  // Fetch necessary fields
                }
            });
